...
def _run_native(dom, eq, m, eps, kw):
    ...
    ldr = dom.generate_loader(bs=kw.get("bs", 2048), meth=kw.get("colloc", "sobol"))
    ...
    bal = AdaptiveLossBalancer(mode=kw.get("ntk_mode", "gradient_norm"), loss_names=["pde","ic","bc"], update_freq=kw.get("ntk_freq", 100)) if kw.get("ntk") else None
    csl = CausalTrainingMixin() if kw.get("csl") else None
    hist = []
    ...
    return {"model": m, "loss_history": hist, "final_loss": hist[-1] if hist else None}
