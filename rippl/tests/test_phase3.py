import torch
import pytest
from rippl.physics.operators import Laplacian, Gradient, Divergence
...
def test_system_validate_fail():
    ...
    dom = Domain(spatial_dims=1, bounds=((0, 1),), resolution=(10,))
    sys = System(equation=eq, domain=dom, fields=["u"])
    with pytest.raises(RipplValidationError) as excinfo:
        sys.validate()
...
def test_field_shape_contract():
    ...
    with pytest.raises(RipplValidationError):
