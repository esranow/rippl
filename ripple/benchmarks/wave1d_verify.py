...
from ripple.models.siren import Siren
from ripple.physics_blocks.residual import HybridWaveResidualBlock
from ripple.core.system import Constraint, Domain
...
def train_time_marching(model_config):
    ...
    model.eval()
    with torch.no_grad():
        u_ripple_final = model(coords_eval).squeeze(-1)
    
    l2_final = torch.norm(u_ripple_final - u_analytic_eval) / torch.norm(u_analytic_eval)
    l2_val = l2_final.item()
    
    return l2_val, u_ripple_final.cpu(), u_analytic_eval.cpu(), all_losses
...
axes[1].set_title("Ripple Solution")
...
plt.tight_layout()
plt.savefig("ripple/benchmarks/wave1d_result.png")
print("\nSaved plot to ripple/benchmarks/wave1d_result.png")
